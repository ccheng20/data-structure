...
bool validateAppend(LinkedListPtr list, bool debug){
    ...
    if(!preSizeMatch) printf("ERROR: (append_pre) Calculated length does not match num_elements. Expected %d, found %llu\n", preLength, list->num_elements);

    if(debug) printf("Testing AppendLinkedList\n");

    AppendLinkedList(list, 100);

    if(debug) printf("Appended 100 to list \n");

    int postLength = calcLength(list, debug);
   
    if(debug) printf("Cached size is %llu, Calculated size is %d \n", list->num_elements, postLength);
    
    bool postSizeMatch = testCachedSizeEqualsCalculatedSize(list->num_elements, postLength);

    if(!postSizeMatch) printf("ERROR: (append_post) Calculated length does not match num_elements. Expected %d, found %llu\n", postLength, list->num_elements);
    ...
    if(debug) printf("Validate Append Complete. List currently size %llu\n", list->num_elements);
...
bool validateRemove(LinkedListPtr list, bool debug){
    ...
    if(debug) printf("Testing RemoveLLElem, input list is size %llu and calculated size is %d. \n", list->num_elements, preLength);
    ...
    if(!sizeMatch) printf("ERROR: Calculated length does not match num_elements. Expected %d, found %llu\n", newLength, list->num_elements);
